...
def scheme_eval(expr, env):
    ...
    if (isinstance(expr, Thunk)):
        return scheme_eval(Pair(expr.body,nil), expr.env)
    return expr
...
def do_cons_stream_form(vals, env):
    ...
    stream"""
    check_form(vals, 2, 2)
    return Stream(scheme_eval(vals[0], env), vals[1], env), None 
...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            "*** YOUR CODE HERE ***"
            frame = self.env.make_call_frame(self.formals, args)
            values = scheme_eval(self.body, frame)
            return values, None
...
class NuProcedure(LambdaProcedure):
    ...
    def evaluate_arguments(self, arg_list, env):
        ...
        for i in range(len(arg_list) - 1, -1, -1):
            body = LambdaProcedure(nil, arg_list[i], env)
            current = Thunk(nil, body, env)
            new_body = Pair(current, new_body)
        return new_body
...
class Thunk(LambdaProcedure):
    ...
    def _symbol(self):
        return 'thunk'
    def apply(self, args, env):
        return self.body.apply(nil, env)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        body = vals.second
        values = do_lambda_form(Pair(target.second, vals.second), env)
        env.define(target.first, values[0])
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    body = vals.second
    if len(vals) > 2:
        k = Pair('begin', vals.second)
        a = function_type(formals, k, env)
        return a, env
    val = function_type(vals[0], body.first, env)
    return val, env
...
def do_let_form(vals, env):
    ...
    check_formals(names)
    "*** YOUR CODE HERE ***"
    f = Pair(intern("lambda"),Pair(names,exprs))
    return Pair(f,values),env
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal):
            raise SchemeError("Invalid symbol given.")
        elif formal in formals_seen:
            raise SchemeError("Repeated symbol.")
        formals_seen.append(formal)
################
# Input/Output #
################
