...
(define (patterns)
  ; YOUR CODE HERE
  (define add_under
    (lambda (lst) (cons 'under lst)))
  (define add_over
    (lambda (lst) (cons 'over lst)))
  (define all
    (cons-stream '() 
      (interleave (stream-map add_over all)
                                 (stream-map add_under all))))
  (define remove_over
    (lambda (lst) (member? 'over lst)))
  (define remove_under
    (lambda (lst) (member? 'under lst)))
  (stream-filter (and remove_over remove_under) all))
...
(define (tree-sums tree)
  ; YOUR CODE HERE
  (define (add-to-all addend val_list)
    (define (add-entry other-number) (+ other-number addend))
    (map add-entry val_list)
  )

  (define (flatten-list deep-list)
    (if (null? deep-list) nil
      (append (car deep-list) (flatten-list (cdr deep-list)))
    )
  )

  (if (null? (children tree)) (cons (entry tree) nil)
    (add-to-all (entry tree) (flatten-list (map tree-sums (children tree))))
  )
)
