...
(define students (* rohin (/ andrew 2)))
students
; expect 5

(define pokemon (lambda (x) (+ x (* x 2))))
(pokemon rohin)
; expect 15

(define pikachu (pokemon andrew))
pikachu
; expect 6
...
(define (take x)
	(if (equal? x 16)
		(quote great)
		(quote notok)))
(take (cs61a (with andrew)))
; expect great

(take (cs61a (with pikachu)))
; expect notok

(define (dont x)
	(if (eqv? x (quote great))
		#t #f))
(dont (take (cs61a (with andrew))))
; expect #t
...
(define anant (become (with pidgeot)))
anant
; expect 35

(define andrew (become (with pikachu)))
andrew
; expect 48

(define paul (become (with andrew)))
paul
; expect 2400

(take (with (- (- (- paul andrew) (- anant rohin)) 2352)))
; expect horrible

(define tired (cons 5 1))
(car tired)
; expect 5
...
(define (map proc items)
  (if (null? items)
      ...
      (cons (proc (car items))
            (map proc (cdr items)))))
;;; it's clipped in the test case given in order to do some further tests

(map with numbers)
; expect (5 6 7 8 9)

(map pokemon numbers)
; expect (15 18 21 24 27)
...
(define (fib n)
	...
	(if (= n 0) 0
		(fib1 0 1 n)))
;;; adapted from the course's pdf in order to test the scheme.py
(fib 10)
; expect 55
